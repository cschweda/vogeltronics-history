...
Two things happen in that pass. **Format:** WebP beats the JPEG photographs and, by a wide margin, the flat-colour box art, which is the wrong kind of image for PNG at this size. There is no `<picture>` fallback — WebP has shipped in every browser since Safari 14 in 2020, and 31 elements of fallback markup is a bad trade. **Size:** every image was being sent far larger than it is ever displayed; the box art arrived 600 px wide to be drawn at 210. Each target is its CSS display width doubled, so a file is right for a 2× screen and no bigger. Together that is **6.5 MB → 1.1 MB on load, an 83% cut.**

Every image also gets a `-full` variant at its natural size and higher quality. Those are for the lightbox — click any photograph, box art or portrait and it opens full size with its caption. The full-size files are a further 2.3 MB in total and are fetched **only on click**, never on load. The lightbox is progressive enhancement: the markup is plain `<img>`, and the click handling, keyboard support (Enter/Space to open, Escape to close), focus management and `role="button"` are all added by script at runtime. With JavaScript off they are simply images.
