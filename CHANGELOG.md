...
- **Credit for the digital re-creations**, in both places a reader might want it: the Epilogue, where "faithful digital reproduction" is first claimed, and the games grid, where anyone arriving from a link lands. Same person as the byline — the history and the homages are one project, and the page had never said so.
...
- **Netlify publishes `dist/`, not the repo root.** Publishing `.` also served `package.json`, `package-lock.json`, `tools/` and all 2.9 MB of full-resolution originals in `assets/img-src/`. None of that is a security problem — the repo is public — but it is five times the deploy weight for no reason, and it offered crawlers a second, larger copy of every photograph. `dist/` now holds exactly what the page loads: `index.html`, the built WebP, the nine voice clips, `og-image.png`, `robots.txt` and `sitemap.xml`. 37 files, 1.9 MB. Paths mirror the repo, so nothing in `index.html` changed, and `npm run dev` now serves `dist/` too — dev and production serve the same bytes.
- **`robots.txt` and `sitemap.xml`.** There was no `robots.txt`, so a request for one got Netlify's HTML 404 page and Lighthouse reported it as invalid — SEO 92. Both are generated by the build; the sitemap's `lastmod` comes from `index.html`'s mtime.
